...
public class TTalgorithm {
    ...
    public static Boolean TTCheckAll(LogicalExpression KB, LogicalExpression alpha, List<String> symbols, HashMap<String, Boolean> model) {
        //System.out.println("Ready to call TTCheckAll(KB, alpha, "+symbols+", "+model+")");
        if(symbols.isEmpty()) {
            if(PLTrue(KB, model)) {
                //System.out.println("The KB model is true, so it returns alpha model "+PLTrue(alpha, model));
                return PLTrue(alpha, model);
            }else {
                //System.out.println("The KB model is false, so it returns true.");
                return true;
            }
        }else {
    ...
    public static void readValue (String input) {
        ...
        if(input.equalsIgnoreCase("a")) {
            ...
            System.out.println("(not Mythical) --> (not Immortal) or (Mammal)");
