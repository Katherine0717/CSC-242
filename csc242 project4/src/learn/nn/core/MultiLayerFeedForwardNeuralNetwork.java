...
public class MultiLayerFeedForwardNeuralNetwork extends FeedForwardNeuralNetwork {
    ...
    public void backprop(Example example, double alpha) {
        ...
        double[][] deltaJ = new double [this.layers.length][];
        NeuronUnit[] outputs = this.getOutputUnits();
        deltaJ[this.layers.length-1] = new double[outputs.length];
        for(int j=0; j<outputs.length; j++){
            double in_j = outputs[j].getInputSum();
            double a_j = outputs[j].activation(in_j);
            double y_j = example.outputs[j];
            deltaJ[this.layers.length-1][j] = outputs[j].activationPrime(in_j)*(y_j-a_j);
        }
        ...
        for(int l = this.layers.length-2; l >= 1; l--){
            deltaJ[l] = new double [this.layers[l].length];
            for(int i = 0; i < this.layers[l].length; i++){
                double in_i = ((NeuronUnit)this.layers[l][i]).getInputSum();
                double g = ((NeuronUnit)this.layers[l][i]).activationPrime(in_i);

                double sum = 0;
                for(int j = 0; j < deltaJ[l+1].length; j++){
                    List<Connection> W_ij = this.layers[l][i].outgoingConnections;
                    sum += deltaJ[l+1][j] * W_ij.get(j).weight;
                }
                deltaJ[l][i] = g * sum;
            }
        }
        ...
        for(int m = 0; m < this.layers.length-1; m++){
            for(int n = 0; n < this.layers[m].length; n++){
                List<Connection> outgoingConnections = this.layers[m][n].outgoingConnections;
                for(int k = 0; k < outgoingConnections.size(); k++){
                    outgoingConnections.get(k).weight = outgoingConnections.get(k).weight + alpha * this.layers[m][n].getOutput() * deltaJ[m+1][k];
                }
            }
        }
